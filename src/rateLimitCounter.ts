...
export async function handleCounterInstall(metadata?: Metadata) {
  try {
    ...
    await scheduler.Schedule({ cron: "0 * * * *", action: {type: "reset_hourly_counter"}}, metadata);
  } catch(e) {
...
export async function resetHourlyCounter(kv: KeyValueStorage, metadata?: Metadata) {
...
export type Counters = [hourCount: number, dailyCount :number, lifetimeCount: number]

export async function queryCounters (kv: KeyValueStorage, metadata?: Metadata) : Promise<Counters> {
...
export async function isAboveRateLimit(kv: KeyValueStorage, maxDaily: number, maxHourly: number, metadata?: Metadata): Promise<boolean> {
  const hourlyCount = Number(await kv.get(HOUR_KEY, metadata) || 0);
  const dailyCount = Number(await kv.get(DAY_KEY, metadata) || 0);
...
export async function incrementCounters(kv: KeyValueStorage, metadata?: Metadata) {
