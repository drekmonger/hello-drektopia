  ...
  return [
    {
      type: 'boolean',
      name: 'acceptance',
      label: 'Click to confirm that your use case conforms with OpenAI and Reddit policies. (Also, you may uncheck this to act like a killswitch, stopping the app from making API calls.)'
    },
...

    {
      type: 'number',
      name: 'temperature',
      label: "Temperature, influences 'creativity' .7 to 1 are sane values. (.8 default): ",
      defaultValue: .8,
      onValidate: (event) => {
        if (event.value! > 2 || event.value! < 0) { return 'Temperature should be between 0 and 2.  .8 is the normal default.'; }
      }
    },
